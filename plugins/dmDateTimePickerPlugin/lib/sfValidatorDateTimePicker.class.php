...
class sfValidatorDateTimePicker extends sfValidatorBase
{
  ...
  protected function configure($options = array(), $messages = array())
  {
    ...
    $this->addOption('date_format', '~(?P<day>\d{2})/(?P<month>\d{2})/(?P<year>\d{4}) (?P<hour>\d{2}):(?P<minute>\d{2})~');
    $this->addOption('with_time', true);
    $this->addOption('date_output', 'd/m/Y H:i');
    $this->addOption('datetime_output', 'd/m/Y H:i');
    $this->addOption('date_format_error', 'd/m/Y H:i');
  ...
  protected function doClean($value)
  {
    ...
    return $value;//isset($date) ? $date->format($format) : date($format, $cleanTime);
  }
