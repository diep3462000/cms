...
class smsActions extends sfActions
{
    ...
    public function executeIndex(sfWebRequest $request)
    {
        ...
        if($shortcode == "9029" && empty($transdate) && empty($moid)){
            $arr_content = explode(' ', $content);
            $user_nap = UserTable::getUserByUserId(end($arr_content));
            if($user_nap){
                http_response_code(200);
                $logger->log('[info]' .  'check user 9029 Viettel hop le|' . end($arr_content));
                echo '1|noi dung hop le';
                die;
            } else {
                $logger_err->log('[err]' .  'user khong ton tai|' . end($arr_content));
                http_response_code(200);
...
//                var_dump(end($arr_content));die;
             //  $user_nap = UserTable::getUserByName(end($arr_content));
                if( ($amount == 1500 &&  strtolower(end($arr_content)) == 'reset') || (strtolower(end($arr_content)) == 'huy') ) {
                    $logger->log('[smsActions] check = verifiedPhone' .   end($arr_content));
...
//                    echo 'message=0|noi dung khong hop le';
                ...
                if(($amount == 1000 || $amount == 1500) && $check_auto_false ==0 ){
                    ...
                    if(strtolower(end($arr_content)) == 'huy'){
                        ...
                        if($amount == 1000 && UserInfoTable::getUserByMsisdn($userid)){
                            $mt_content = 'So+dien+thoai+nay+da+duoc+su+dung+de+kich+hoat+cho+Id+' . $user_nap->getUserId() . ".De+huy+kich+hoat+soan+tin+BKG+HUY+gui+8098";
                        } else {
                            $mt_content = 'Ma+xac+thuc+tai+khoan+bigken+cua+ban+la+'.$code.'+.Ma+xac+thuc+chi+co+gia+tri+trong+90+phut';
                        }
                    }
                    $verify->setType($type); // 0 lã xác thực, 1 ; 1 ;à reset mật khẩu ; 2 hủy xác thực
                    $verify->setStatus(0);
                    $verify->save();
                } else {
        ...
        $logger_err->log('[error] [null] request  ' . $content);
        http_response_code(400);
        echo 'requeststatus=2';
        die;
    }
