<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_gv_manage_provider" style="margin-top: 5px !important;">
        <thead>
        <tr>
<!--            <th id="sf_admin_list_batch_actions" style="width: 10px">-->
<!--                <input id="sf_admin_list_batch_checkbox" type="checkbox" onclick="checkAll();" /></th>-->

            <?php include_partial('gvManageProvider/list_th_tabular', array('sort' => $sort)) ?>
        ...
        <?php else: ?>
            ...
            <!--   End diepth2   -->
            <?php foreach ($results as $i => $provider): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>

                <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $provider->getPrimaryKey() ?>}" rel="<?php echo $provider->getPrimaryKey() ?>">

                    <?php #include_partial('gvManageProvider/list_td_batch_actions', array('provider' => $provider, 'helper' => $helper)) ?>
                    <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                    <?php include_partial('gvManageProvider/list_td_tabular', array('provider' => $provider, 'orderNo' => $orderNo)) ?>
                    <?php include_partial('gvManageProvider/list_td_actions', array('provider' => $provider, 'helper' => $helper)) ?>
                </tr>
            <?php endforeach; ?>
            </tbody>
            <tfoot>
            <tr>
                <th colspan="5">
                    <?php if ($pager->haveToPaginate()): ?>
                        <div style="position: relative; width: auto; float:right"><?php include_partial('gvManageProvider/pagination', array('pager' => $pager)) ?></div>
